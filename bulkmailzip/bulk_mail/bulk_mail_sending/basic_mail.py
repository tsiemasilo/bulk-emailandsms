...
# smtp - https://www.devdungeon.com/content/read-and-send-email-python
...
# s_password = os.getenv("PASSWD", "J6nitor-123")
...
def connect_smtp_server(bl_secure:bool = True, bl_starttls:bool = True, bl_debug:bool = False):
    ...
    try:
        ...
        if bl_debug: o_server.set_debuglevel(1) # Show SMTP server interactions
...
def imap_tools_mailbox(bl_mailbox:bool = False) -> tuple:
    ...
    t_out = ()
    d_messages = {}
    mailbox = MailBox(s_mail_server).login(s_username, s_password)
    # mailbox.fetch(H("header-key", "header-value"))...?
    for msg in mailbox.fetch():# for s_id in list(mailbox.uids()):
        try:
            d_messages[msg.uid] = msg # list(mailbox.fetch(s_id))[0]
        except Exception as exc:
            except_output()
            breakpoint()
        # end of try-except
    # end of looping through messages
    if bl_mailbox:
        return (d_messages, mailbox)
    else:
        mailbox.logout()
        del mailbox
        return (d_messages)
    # end of checking if return should include mailbox
...
def cls():
    ...
    # end of checking OS type
    return None
# end of cls function


class ActionType(Enum):
    """e-mail interaction action types enumerator values"""
    LIST = "list"
    SEND = "send"
    VIEW = "view"
    DELETE = "delete"
# end of ActionType enumerator class
...
def check_mails():
    ...
    d_mails, mailbox = imap_tools_mailbox(bl_mailbox=True)
    def fetch_mails() -> dict:
        d_messages = {}
        for msg in mailbox.fetch(): # s_id in list(mailbox.uids()):
            try:
                d_messages[msg.uid] = msg # list(mailbox.fetch(s_id))[0]
            except Exception as exc:
                except_output()
        ...
        return d_messages
    # end of inner function fetch_mails
    def list_mails():
        print(f"total of {len(d_mails)} mails")
        for sk, o_mail in d_mails.items():
            print(f"{sk} - {o_mail.subject}")
        # end of looping through mail items
    # emd of inner function list_mails
    list_mails()
    bl_continue = True
    while bl_continue:
        print("[l]ist all mails, [s]end new mail, [v]iew mail, [d]elete mai(s), [c]lear mailbox, [e]xit")
        s_choice = str(input("Action?")).strip().lower()
        if s_choice in ["l", "s", "v", "d", "c"]:
            match s_choice:
                case "l":
                    d_mails = fetch_mails()
                    list_mails()
                # end of l case
                case "v":
                    s_id = str(input("ID?")).strip()
                    if s_id in d_mails.keys():
                        msg = d_mails[s_id]
                        print(msg.subject)
                        print(strip_tags(msg.html))
                        s_nada = input("\n\n...hit enter to continue...")
                    # end of checking for key match
                # end of v case
                case "d":
                    s_id = str(input("single ID, or multiple comma-separated?")).strip().replace(" ", "")
                    if s_id.count(",")>0:
                        l_ids = s_id.split(",")
                        print(f"total of {len(l_ids)} messages")
                        s_confirm = str(input("Are you sure? (y/n)")).strip().lower()
                        if s_confirm=="y":
                            s_ids = ",".join(l_ids)
                            mailbox.delete(s_ids, chunks=5)
                        ...
                        if s_id in d_mails.keys():
                            ...
                            # end of confirmation check
                        # end of checking for key match
                    # end of checking if multiple or single mail selected
                # end of d case
                case "c":
                    l_ids = list(d_mails.keys())
                    print(f"total of {len(l_ids)} messages")
                    s_confirm = str(input("Are you sure? (y/n)")).strip().lower()
                    if s_confirm=="y":
                        s_ids = ",".join(l_ids)
                        mailbox.delete(s_ids)
                    # end of confirmation check
                # end of c case
                case "s":
                    s_to = str(input("e-mail address to?")).strip()
                    try:
                        s_to = validate_email(s_to).email
                        s_subject = str(input("subject?")).strip()
                        bl_html = True if str(input("[h]tml or [p]lain text?")).strip().lower()=="h" else False
                        s_format = "HTML" if bl_html else "plain text"
                        print("Enter body of message in {s_format} - use ctrl+Z under windows, or ctrl+D under linux to end multi-line input")
                        s_body = "".join(sys.stdin.readlines())
                        try:
                            bl_success = False
                            print("sending...")
                            if bl_html:
                                bl_success = smtp_send_html(l_to=[s_to], s_subject=s_subject, s_body=s_body, s_from=s_username, s_from_name="", bl_secure=True)
                            else:
                                bl_success = smtp_send_plain(l_to=[s_to], s_subject=s_subject, s_body=s_body, s_from=s_username, s_from_name="", bl_secure=True)
                            # end of checking if html or plain
                            if bl_success:
                                s_nada = input("mail sent! (hit enter to continue)")
                            # end of checking for boolean response
                        except Exception as send_exc:
                            print(f"issue sending mail - {send_exc}")
                        # end of try-except around sending mails
                    except Exception as exc:
                        print(f"{s_to} is not a valid e-mail address")
                    # end of try-except validating email format|syntax
                # end of s case
                case _: # default for no reason
                    d_mails = fetch_mails()
                    bl_continue = False
                # end of _ case
            # end of match against s_choice
        else:
            d_mails = fetch_mails()
            bl_continue = False
        # end of making sure action is interactive/valid
        cls()
